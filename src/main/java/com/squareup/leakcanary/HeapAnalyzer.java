...
public final class HeapAnalyzer {
  ...
  public AnalysisResult[] checkLeakByClass(File heapDumpFile, String clsName) {
    long analysisStartNanoTime = System.nanoTime();

    if (!heapDumpFile.exists()) {
      Exception exception = new IllegalArgumentException("File does not exist: " + heapDumpFile);
      return new AnalysisResult[] { failure(exception, since(analysisStartNanoTime))};
    }
    ...
    try {
      ...
      ClassObj clsObj = snapshot.findClass(clsName);
      if (clsObj!=null && clsObj.getInstanceCount()>0) {
        List<Instance> instanceList = clsObj.getInstancesList();
        AnalysisResult[] results = new AnalysisResult[instanceList.size()];
        for (int i = 0; i < instanceList.size(); i++) {
          Instance instance = instanceList.get(i);
          results[i] = findLeakTrace(analysisStartNanoTime, snapshot, instance);
        }
        return results;
      }

      // False alarm, weak reference was cleared in between key check and heap dump.
      return new AnalysisResult[] { noLeak(since(analysisStartNanoTime))};

    } catch (Throwable e) {
      return new AnalysisResult[] { failure(e, since(analysisStartNanoTime))};
    }
  }
